...
function readCookiePairAttribute(key: string, value: string): O.Option<CookieAttribute> {
...
export function parseAttributes(attributes: string[]): E.Either<string, CookieAttribute>[] {
...
function reduceCookieAttributes(cookie: Cookie, attributes: CookieAttribute[]): Cookie {
...
export function parseSetCookie(
    ...
    const { left: errors, right: cookieAttributes } = A.separate(parseAttributes(attributes))
...
export function applyCookieToCookies(
...
export function splitPair(sep: string, keyVal: string): O.Option<readonly [string, string]> {
    return pipe(
        O.fromNullable(
            new RegExp(`(.*?)${sep}(.*)`)
                .exec(keyVal)),
