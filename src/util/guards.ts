...
// export function partitionFst<A, B extends A, C extends A>(
//   parition: (a: A[]) => { left: B[]; right: C[] },
// ): <F>(tuples: readonly [A, F][]) => { left: readonly [B, F][]; right: readonly [C, F][] } {
//   return tuples => {
//     const { left, right } = parition(tuples.map(t => t[0]));

//     return {
//       left: tuples.map((t, i) => [left[i], t[1]] as const),
//       right: tuples.map((t, i) => [right[i], t[1]] as const),
//     };
//   };
// }
...
export const guardThird =
  <A, B, C, F extends C>(refinement: Refinement<C, F>) => (tuple: [A, B, C]): tuple is [A, B, F] =>
    refinement(tuple[2]);
