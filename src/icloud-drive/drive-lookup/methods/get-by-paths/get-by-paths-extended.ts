...
import { filterOrElse, map, Monad } from '../..'
...
export const getByPathsStrictDocwsroot = (
  paths: NEA<NormalizedPath>,
): Monad<NEA<T.DetailsOrFile<T.DetailsDocwsRoot>>> => {
  return pipe(
...
export const getByPathFolderStrictDocwsroot = (
  path: NormalizedPath,
): Monad<T.DetailsDocwsRoot | T.NonRootDetails> =>
  pipe(
    getCachedDocwsRoot(),
...
export const getByPathsFoldersStrictDocwsroot = (
  paths: NEA<NormalizedPath>,
): Monad<NEA<T.DetailsDocwsRoot | T.NonRootDetails>> =>
  pipe(
...
export const getByPath = <R extends T.Root>(root: R, path: NormalizedPath): Monad<V.GetByPathResult<R>> => {
  return pipe(
...
export const getByPathsDocwsroot = (paths: NEA<NormalizedPath>): Monad<NEA<V.GetByPathResult<T.DetailsDocwsRoot>>> => {
...
export const getByPathDocwsroot = (path: NormalizedPath): Monad<V.GetByPathResult<T.DetailsDocwsRoot>> => {
  return pipe(
